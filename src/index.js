...
class TencentThinkPHP extends Component {
  ...
  async prepareInputs(inputs = {}) {
    inputs.name =
      ...
      `ThinkPHPComponent_${random({ length: 6 })}`
  ...
  async default(inputs = {}) {
    ...
    inputs.fromClientRemark = inputs.fromClientRemark || 'tencent-thinkphp'
    const tencentCloudFunctionOutputs = await tencentCloudFunction(inputs)
    const apigwParam = {
      serviceName: inputs.serviceName,
      description: 'Serverless Framework tencent-thinkphp Component',
    ...
    apigwParam.fromClientRemark = inputs.fromClientRemark || 'tencent-thinkphp'
  ...
  async remove(inputs = {}) {
    ...
    const removeInput = {
      fromClientRemark: inputs.fromClientRemark || 'tencent-thinkphp'
    }
    const tencentCloudFunction = await this.load('@serverless/tencent-scf')
    const tencentApiGateway = await this.load('@serverless/tencent-apigateway')
...
module.exports = TencentThinkPHP
